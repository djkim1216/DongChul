...
$(function(){
	...
	var mapLoad = function(){
		...
		routeXArr.forEach(function(val, index){
			...
			kakaoMarker.push({
				content : `<div class="mapInfoText">${place_nameArr[index]}</div>`,
		...
		routeXArr.forEach(function(val, index){
			...
			var circle = new kakao.maps.Circle({
				center : new kakao.maps.LatLng(routeYArr[index],val),  // 원의
																		// 중심좌표
																		// 입니다
			...
			if(category_codeArr[index] === 'FD6'){ // 맛집
				markerImage = new kakao.maps.MarkerImage(resMarker.src, resMarker.size, resMarker.option);
			} else if (category_codeArr[index] === 'AT4') { // 명소
				markerImage = new kakao.maps.MarkerImage(tourMarker.src, tourMarker.size, tourMarker.option);
			} else if (category_codeArr[index] === 'AD5') { // 숙소
				markerImage = new kakao.maps.MarkerImage(roomsMarker.src, roomsMarker.size, roomsMarker.option);
			}
			
			var marker = new kakao.maps.Marker({
		        map: map, // 마커를 표시할 지도
		        image : markerImage,
		        position: kakaoMarker[index].latlng, // 마커의 위치
		        index : index  // 마커 인덱스 생성
		    });
			...
			var routeCount= $('<a href="#" onclick="return false"><span class="routeIndex">'+count+'</span></a>');
			
			$('div.mapInfoText').parent().parent().parent().parent().find('div > div > img').eq(index).before(routeCount);
			...
			kakao.maps.event.addListener(marker, 'mouseover', function() {
				$('div.mapInfoText').parent().parent().css({
					"transition-duration": "0.3s",
					"display" : "none"
				})
				$('div.mapInfoText').eq(index).parent().parent().css({
					"transition-duration": "0.3s",
					"display":"block"
				});
			});
			...
			kakao.maps.event.addListener(marker, 'click', function() {
				fnMove($('div.dayInfoBox').eq(index));
				
			});
			
			
			$('span.routeIndex').eq(index).click(function(){
				fnMove($('div.dayInfoBox').eq(index));
			});
			
			
			$('span.routeIndex').eq(index).hover(function(){
				$('div.mapInfoText').eq(index).parent().parent().css({
					"transition-duration": "0.3s",
					"display":"block"
				});
		...
		$('div.mapInfoText').parent().parent().find('div:nth-child(1)').css("margin-top","6px");
		$('div.mapInfoText').css({
			"margin-top": "2px",
	    	"font-weight": "100",
	    	"text-indent": "5px"
	...
	var routeListLoad = function(){
		...
		$.ajax({
			...
			beforeSend : function(){
				var loading = `<div class="loading"><div class="sk-fading-circle">
  ...
  <div class="sk-circle12 sk-circle"></div>
				...
				$('article.contents').children('div.loading').remove();
			}
		});
	}
